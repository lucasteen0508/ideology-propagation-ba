...
# Node2Vec-Spalte umwandeln (String -> NumPy-Array)
...
print("Feature-Dimension vor Scaling:", len(X_combined[0]))

# Standardisierung für RBF-Kernel und um Vergleich mit PCA zu garantieren
X_scaled = StandardScaler().fit_transform(X_combined)

# PCA fitten ohne feste n_components (alle Komponenten)
pca = PCA()
pca.fit(X_scaled)
...
# Anzahl der Komponenten bestimmen, die mehr als 95% der Varianz erklären
n_components_95 = (cumulative_variance >= 0.95).argmax() + 1
print(n_components_95)

# PCA mit reduzierter Komponentenanzahl fitten
pca = PCA(n_components=n_components_95, random_state=42)
X_pca = pca.fit_transform(X_scaled)

# Plot für kumulative erklärte Varianz durch PCA
...
for cls, indices in seed_indices_per_class.items():
    holdout_size = int(0.2 * len(indices))  # 20% Holdout
...
param_grid = {'kernel': ['rbf', 'knn'],
              ...
              # 'n_neighbors': [4,5,6,7,8] # aktuell auskommentiert

best_acc = 0
best_params = None
best_model = None

# Manuelle Grid Search mit Progress-Bar
param_combinations = [
    (kernel, gamma) # n_neighbors nicht berücksichtigt
    for kernel in param_grid['kernel']
    for gamma in param_grid['gamma']
]

for kernel, gamma in tqdm(param_combinations, desc="Grid Search", unit="combination"):
    # LabelPropagation-Labels vorbereiten
    y_lp = -1 * np.ones_like(y)
    y_lp[train_seed_indices] = y[train_seed_indices]

    # Modell initialisieren (mit PCA-Reduktion!)
    ...
    lp_model.fit(X_pca, y_lp)

    # Holdout Vorhersagen
    y_holdout_pred = lp_model.transduction_[holdout_indices]

    # Accuracy auf Holdout berechnen
...
plot_evaluation_labels(y_holdout_true, y_holdout_pred, title_suffix="Eval. Holdout Seed Users (S-Bert/Node2Vec mit PCA)")

# Predictions für alle User
y_pred_all = best_model.transduction_
# Wahrscheinlichkeiten für Klassenzugehörigkeit der Knoten speichern
y_proba_all = best_model.predict_proba(X_pca)
...
print("Fertig! Predictions und Confidence sind im Dataset.")


# Export der Klassifikationen
export_df = dataset.copy()
export_df = export_df.drop(columns=['bert_embeddings'], errors='ignore')  # Optional: Embeddings weglassen
#export_df.to_csv('user_labelprop_predictions_full_pca.csv', index=False)
